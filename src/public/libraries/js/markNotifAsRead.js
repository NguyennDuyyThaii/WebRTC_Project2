    ...
    $.ajax({
        ...
        success: function(result) {
            if (result) {
                ...
                decreaseNumberNotification("noti_counter", targetUsers.lenngth)
            }
        }
    })
}
$(document).ready(function() {
    $('#popup-mark-notif-as-read').bind('click', function() {
        let targetUsers = []
        $(".noti_content").find("div.notif-readed-false").each(function(index, value) {
            targetUsers.push($(value).data("uid"))
        })
    ...
    $('#modal-mark-notif-as-read').bind('click', function() {
        let targetUsers = []
        $("ul.list-notifications").find("li>div.notif-readed-false").each(function(index, value) {
            targetUsers.push($(value).data("uid"))
        })
        if (!targetUsers.length) {
            alertify.notify("Bạn không còn thông báo nào chưa đọc", "error", 7)
